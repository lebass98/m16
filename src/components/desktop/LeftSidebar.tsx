...
export default function LeftSidebar({
  ...
  return (
    <>
      <Box className="app-left-nav" sx={{ width: collapsed ? 88 : 260, transition: 'width 0.25s ease', flexShrink: 0, bgcolor: 'background.paper', backdropFilter: 'var(--glass-blur)', WebkitBackdropFilter: 'var(--glass-blur)', color: 'text.primary', display: 'flex', flexDirection: 'column', position: 'sticky', top: 0, height: '100vh', overflowY: 'auto', overflowX: 'hidden', borderRight: '1px solid rgba(255,255,255,0.18)' }}>
        <Tooltip title={collapsed ? siteTitle : ''} placement="right" arrow>
          <Box onClick={onOpenSiteModal} sx={{ display: 'flex', alignItems: 'center', gap: '12px', m: '16px', p: '12px', borderRadius: '12px', bgcolor: 'rgb(var(--palette-grey-500Channel) / 0.08)', cursor: 'pointer', justifyContent: collapsed ? 'center' : 'flex-start', '&:hover': { bgcolor: 'rgb(var(--palette-grey-500Channel) / 0.16)' } }}>
        ...
        <Box sx={{ p: '0 16px', display: 'flex', flexDirection: 'column', gap: '2px' }}>
          ...
          <Tooltip title={collapsed ? 'Pages' : ''} placement="right" arrow>
            <Box className="reveal-right" style={{ animationDelay: '60ms' }} sx={{
              ...
              '&::before': !collapsed ? { content: '""', position: 'absolute', left: 0, top: '50%', transform: 'translate(-16px,-50%)', width: 3, height: 20, borderRadius: '0 2px 2px 0', bgcolor: 'primary.main' } : {},
            }}>
          ...
          {!collapsed && (
            <>
              ...
              {depth1Categories.map(({ key, count }, i) => {
                ...
                const label = key || '(미분류)';
                return (
                  <Box key={key || `__empty_${i}`} onClick={() => onToggleSectionFilter(key)} className="reveal-right" style={{ animationDelay: `${260 + i * 40}ms` }} sx={{
        ...
        <Box sx={{ flex: 1 }} />
        <Box sx={{ m: '16px', p: '16px', borderRadius: '16px', bgcolor: 'rgb(var(--palette-primary-mainChannel) / 0.08)', display: 'flex', alignItems: 'center', justifyContent: collapsed ? 'center' : 'flex-start', gap: '12px' }}>
