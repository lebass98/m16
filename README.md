# URL Index — 페이지 진행도 대시보드

웹 프로젝트의 페이지 목록을 한 곳에서 관리하고, 각 페이지의 **진행도(PC/모바일)** · **메뉴 카테고리** · **업데이트 이력** · **실시간 미리보기**까지 한 화면에서 확인할 수 있는 React 기반의 인덱스 대시보드입니다.
...
- **메타데이터** — 페이지 제목, ID, URL 경로, 생성일/최근 업데이트일/완료일, 비고를 함께 저장.

### 뷰
- **데스크탑**
  - **리스트(DataGrid)** — `@mui/x-data-grid` 기반 표 형태.
  - **썸네일 그리드** — 한 줄에 2 · 3 · 4 · 5개 카드 토글, 카드 안에서 PC / 태블릿 / 모바일 미리보기 비율 전환.
  - **좌측 사이드바** — 워크스페이스 정보, 페이지 카운트, **카테고리(depth1)별 필터링**, 사이드바 접기/펼치기.
  - **우측 패널** — 전체 진행도, 최근 업데이트 활동, 북마크, 섹션별 완성도 요약.
- **모바일**
  - **풀-스크린 카드 스와이프** — `scroll-snap`으로 페이지 단위 가로 스와이프.
  - **섹션 도트 인디케이터** — 현재 섹션 위치 시각화.
...
- **검색** — `Cmd/Ctrl + K`로 빠른 검색. 제목 · ID · depth · 비고를 모두 매칭.
- **정렬** — No / 페이지제목 / ID / 경로 / 진행도 / 생성일 / 최근 업데이트 / 완료일.
...
- **Google Sheets CSV** — `SiteConfig.sheetCsvUrl`을 지정하면 시트에서 데이터를 fetch. 실패 시 정적 데이터로 자동 fallback.

### 테마 · 접근성
- **다크 모드** — 토글 즉시 전환, 설정 영구 저장.
- **컬러 프리셋** — Minimals UI의 다양한 primary 컬러 프리셋 적용.
...
├── scripts/
│   └── export-to-csv.ts          # tableData → CSV 내보내기
├── src/
...
export interface SiteConfig {
  ...
  data: TableSection[];       // 정적 데이터 (fallback)
  color?: string;             // 사이트 강조 컬러
  description?: string;
  /**
   * Google Sheets "웹에 게시" CSV URL.
   * 지정 시 앱 로드와 함께 시트에서 fetch (실패하면 data로 fallback).
   */
  sheetCsvUrl?: string;
}
```
...
### 컬러 프리셋
설정 드로어 → Preset 선택. `src/theme/presets.ts`에서 커스텀 프리셋 추가 가능.

### 폰트
설정 드로어 → Font Family 선택. `src/theme/fonts.ts`에 폰트 정의.
모든 폰트는 `@fontsource/*` 패키지로 셀프 호스팅 — 외부 네트워크 의존 없음.

### 폰트 크기
설정 드로어의 슬라이더로 12–20px 조정. `document.body.style.zoom`을 이용해 전체 콘텐츠를 비율로 확대/축소.
...
`scripts/export-to-csv.ts`가 `tableData`를 읽어 `exports/` 폴더에 CSV 파일로 출력합니다.
...
2. 게시 대상: 특정 시트 선택
3. 형식: **쉼표로 구분된 값(.csv)**
4. **게시** → 발급된 URL을 `SiteConfig.sheetCsvUrl`에 지정
...
| 데이터       | 정적 TS · Google Sheets CSV                                                |
| 영구 저장    | `localStorage` (다크모드/뷰모드/북마크/정렬/프리셋/폰트 등)                |
| 빌드 도구    | Vite 8 (Rolldown 기반), TypeScript Project References                      |
| 린트         | ESLint 9, `typescript-eslint`, `eslint-plugin-react-hooks`                 |
| 도구         | `tsx` (CSV 익스포트 스크립트 실행)                                         |
...
`backdrop-filter`, `scroll-snap-type`, `aspect-ratio`, CSS Grid를 적극 사용하므로 모던 브라우저 권장.
