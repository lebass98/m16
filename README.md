...
### 도움말 가이드 (온보딩 투어)
- **첫 진입 가이드** — 사용자가 처음 대시보드에 접속했을 때 또는 설정창의 '도움말 가이드 다시 보기' 버튼을 클릭했을 때 앱의 주요 UI 요소들을 단계별로 안내합니다.
- **단계별 가이드 구성**:
  1. 워크스페이스 전환 (사이트 선택)
  2. 카테고리 & 섹션 필터 (사이드바)
  3. 진행율 범위 & 상세 검색 (필터 바)
  4. 카드 정보 및 액션 버튼 (카드 리스트)
  5. 종합 진행도 및 활동 내역 (우측 패널)
  6. 설정 메뉴 (헤더 설정 아이콘)
  7. 설정 패널 & 화면 개인화 (설정창 드로어)
- **설정창 자동 제어 연동** — 7단계(설정 패널)로 진입 시 우측 설정창(Drawer)이 자동으로 열리며, 이전 단계로 되돌아가거나 가이드를 완료/건너뛰기하면 자동으로 닫힙니다.
...
### 온보딩 도움말 가이드 제어
- **가이드 완료 여부 저장**: 온보딩 완료 여부는 `localStorage.hasCompletedOnboarding = 'true' | 'false'`에 저장됩니다 (`usePersistedState` 훅 활용). 이 값을 강제로 지우거나 `false`로 설정하면 다음 로드 시 가이드가 자동으로 다시 나타납니다.
- **로컬 스토리지 키**: `hasCompletedOnboarding`
